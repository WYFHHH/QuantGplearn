...
# 并行实现子树交叉，变异
def _parallel_evolve(max_length, n_programs, parents, X, y, sample_weight, seeds, params, security_data=None,
                     time_series_data=None, first_train=False, add_elite_thread=False, last_train_elites=None):
    ...
    # Build programs
    if not first_train:
        if not add_elite_thread:
            programs = []
        else:
            programs = []
            for p in range(len(last_train_elites)):
                programs.append(last_train_elites[p])
    else:
        programs = []
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.last_train_elites = last_train_elites
        self.true_population_size = self.population_size

        if last_train_elites != None:
            self.reduced_population_size = self.population_size - len(self.last_train_elites)

        self.tolerable_corr = tolerable_corr

        # from original gplearn
        self.n_features_in_ = ...
        self._function_set = ...
        self._arities = ...
    ...
    # fit 的时候考虑时序问题
    # 转移出数据处理模块
    def fit(self, X, y, max_length, sample_weight=None):
        ...
        print(f"tolerable_corr: {self.tolerable_corr}")
        # 检查数据类型
        ...
        for gen in range(prior_generations, self.generations):
            ...
            if gen == 0 and self.first_train:
                parents = None
            elif gen == 0 and not self.first_train:
                parents = None
                self.population_size = self.reduced_population_size
            else:
                try:
                    parents = self._programs[gen - 1]
                    # print(f"parents: {parents}")
                    self.population_size = self.true_population_size
                except:
            ...
            population = Parallel(n_jobs=n_jobs, temp_folder=None, verbose=int(self.verbose > 1))(
                delayed(_parallel_evolve)(
                    ...
                    add_elite_thread=True if i == 0 else False,
                    last_train_elites=self.last_train_elites
                )
            ...
            if len(population) < self.hall_of_fame:
                print(f"父代种群数量小于hall_of_fame，停止迭代")
                break

            fitness = [(program.raw_fitness_ - 1.01 ** program.length_ + 1) for program in population]
            ...
            if self._metric.greater_is_better:
                ...
                # 找到第 10 大的适应度的索引
                parent_index = sorted_indices[-self.hall_of_fame]  # 负数索引表示从后往前数，第 10 大就是倒数第 10 个
                # 获取第 10 大适应度对应的父代个体
                best_fitness = fitness[parent_index]
                if best_fitness >= self.stopping_criteria:
                    print(datetime.datetime.now(),
                          f"greater_is_better 早停，best_fitness {best_fitness} 大于 stopping_criteria: {self.stopping_criteria}")
                    break
        ...
        if isinstance(self, TransformerMixin):
            ...
            # evaluation = np.array([gp.execute(X) for gp in tqdm.tqdm([self._programs[-1][i] for i in hall_of_fame])])

            # 这里对gplearn源码做了局部的多进程优化
            def gp_ex(gp, idx):
                return {idx: gp.execute(X)}

            pool = mp.Pool(mp.cpu_count() - 2)
            gp_res_list = []
            indices_list = []
            for idx, gp in enumerate([self._programs[-1][i] for i in hall_of_fame]):
                # 在提交任务时记录索引
                gp_res_list.append(pool.apply_async(gp_ex, (gp, idx)))
                indices_list.append(idx)
            pool.close()
            pool.join()
            sort_res_dict = {}
            for i in sorted(indices_list):
                data_dict = gp_res_list[i].get()
                sort_res_dict[list(data_dict.keys())[0]] = list(data_dict.values())[0]
            sorted_dict = {k: sort_res_dict[k] for k in sorted(sort_res_dict)}
            evaluation = np.array(list(sorted_dict.values()))
            ...
            while len(components) > self.n_components:
                # if iii % 100 == 0:
                #     print(datetime.datetime.now(), f"剔除相关性进度: {iii}, {len(components)}, {self.n_components}")
...
class SymbolicTransformer(BaseSymbolic, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # 使用 joblib 进行并行计算
        result = Parallel(n_jobs=self.n_jobs)(delayed(execute_gp)(gp, X) for gp in self._best_programs)
